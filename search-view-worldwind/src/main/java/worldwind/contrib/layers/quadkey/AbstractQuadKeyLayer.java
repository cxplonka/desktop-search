...
public abstract class AbstractQuadKeyLayer extends AbstractLayer {
    ...
    protected synchronized void renderTile(DrawContext dc, String tileKey, final String tileURL, Sector sector) {
        ...
        try {
            QuadKeyEarthTile veTile = loadingTiles.containsKey(tileKey)
            ...
            if (isTileInMemory(tileKey)) {
                ...
                // tile on disk?
                URL fileUrl = WorldWind.getDataFileStore().findFile(tileCachePath, false);

                if (fileUrl != null) {
                    // Yes, load from disk
                    tile = new TextureTile(sector);
                    tile.setTexture(dc.getTextureCache(), TextureIO.newTexture(fileUrl, true, null));
    ...
    public Sector QuadKeyToSector(String quadKey) {
        ...
        return new Sector(Angle.fromDegrees(lat), Angle.fromDegrees(lat2), Angle.fromDegrees(lon), Angle.fromDegrees(lon2));
    }
