/** *****************************************************************************
 * Copyright (c) 2006 Vladimir Silva and others. All rights reserved. This
 * program and the accompanying materials are made available under the terms of
 * the Eclipse Public License v1.0 which accompanies this distribution, and is
 * available at http://www.eclipse.org/legal/epl-v10.html
 *
 * Contributors: Vladimir Silva - initial API and implementation
 ****************************************************************************** */
 ...
 *
 * MS VE uses a mercator projection which significantly distorts scale and area
 * (particularly near the poles).
 *
 * To optimize the performance of map retrieval and display, the rendered map is
 * cut into tiles of 256 x 256 pixels each. As the number of pixels differs at
 * each level of detail, so does the number of tiles:
 *
 * To optimize the indexing and storage of tiles, the two-dimensional tile XY
 * coordinates are combined into one-dimensional strings called quadtree keys,
 * or �quadkeys� for short. Each quadkey uniquely identifies a single tile at a
 * particular level of detail.
 *
 * This layer works by computing the quad key and the lat/lon sector of the eye
 * position tile and the rendering neighbor tiles in a circular pattern.
 *
 * The VE projection (Mecator) doesn't match the ellipsoidal lat/lon projection
 * of World Wind, however by computing the sector of the VE tile and rendering
 * using the {@link SurfaceTileRenderer} the original image is warped to fit the
